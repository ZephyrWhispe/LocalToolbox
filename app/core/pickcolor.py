...
ZOOM = 9  # 默认放大倍数
ZOOM_MIN, ZOOM_MAX = 4, 24  # 可调放大倍数范围
_SIDE = 17  # 采样区域边长（奇数，含中心点在中央）
_SIZE = _SIDE * ZOOM  # 默认放大后显示边长像素
_SAMPLE_MS = 30  # 采样间隔


def _clamp_zoom(value):
    """把放大倍数钳位到 [ZOOM_MIN, ZOOM_MAX]，非法值回退默认。"""
    try:
        z = int(value)
...
class PickColor:
    ...
    def __init__(self, log=None, zoom=None):
        self._log = log or (lambda m: None)
        self._zoom = _clamp_zoom(ZOOM if zoom is None else zoom)
        self._size = _SIDE * self._zoom
    ...
    def run_blocking(self, copy_to_clipboard=True):
        ...
        # 中央放大画布（左上角小窗展示）
        preview = tk.Canvas(
            root, width=self._size, height=self._size,
            bg="#111111", highlightthickness=0)
        preview.place(x=self._size // 4, y=self._size // 4)
        info = tk.Label(
        ...
        def sample():
            try:
                ...
                zoom = img.resize((self._size, self._size), Image.NEAREST)
                img_tk = _photo(zoom)
                ...
                # 十字准线
                mid = self._size / 2
                preview.create_line(mid, 0, mid, self._size, fill="#ffffff", dash=(2, 2))
                preview.create_line(0, mid, self._size, mid, fill="#ffffff", dash=(2, 2))
                hexc = rgb_to_hex(*px)
                info.config(
                    text="R %3d   G %3d   B %3d\n%s    %s"
...
def pick_color_blocking(copy_to_clipboard=True, zoom=None):
    """便捷入口：默认复制十六进制到剪贴板；返回 {rgb,hex} 或 None。

    zoom：放大倍数（4–24），None 用默认 9。
    """
    return PickColor(zoom=zoom).run_blocking(copy_to_clipboard=copy_to_clipboard)
